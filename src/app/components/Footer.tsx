...
const SOCIALS = [
  {
    ...
    hover: "group-hover:text-fuchsia-400",
      ...
      <div className="container relative mx-auto max-w-7xl px-6 py-12">
        <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
          ...
          <nav className="flex flex-col space-y-2 text-sm">
            {NAV_LINKS.map(({ href, label }) => (
              <Link
                key={href}
                href={href}
                className="transition-colors duration-200 hover:text-fuchsia-400"
              >
                {label}
              </Link>
            ))}
          ...
          <div className="flex items-center gap-4 md:justify-end">
            {SOCIALS.map(({ href, icon, hover, glow }) => (
              <Link
                key={href}
                href={href}
                target="_blank"
              ...
              >
                <span
                  className={`absolute inset-0 -z-10 rounded-full ${glow} opacity-0 blur-md transition group-hover:opacity-100`}
                />
                <span className={`group-hover:${hover}`}>{icon}</span>
              </Link>
            ))}
          </div>
        </div>
